...
const createCustomer = async (req, res) => {
  const { firstName, lastName, nic, contact, email, password } = req.body;
  ...
  try {
    ...
    const newCustomer = new Customer({
      firstName, lastName, nic, contact, email, password: hashedPassword, confirmPassword: hashedPassword, 
    });
...
const loginCustomer = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: 'Login successful', token, userId: customer._id, 
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
