...
const UpdateTicket = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const numericFields = [
      ...
      { field: 'ticketReleaseRate', label: 'Ticket Release Rate' },
      { field: 'customerRetrievalRate', label: 'Customer Retrieval Rate' },
      { field: 'maxTicketCapacity', label: 'Max Ticket Capacity' },
      { field: 'price', label: 'Price' },
      { field: 'releaseInterval', label: 'Release Interval (ms)' },
      { field: 'retrievalInterval', label: 'Retrieval Interval (ms)' },
    ];
  
    numericFields.forEach(({ field, label }) => {
      const value = parseFloat(ticketData[field]); // Parse as a number
      if (isNaN(value) || value < 0 || value > 10000000) {
        validationErrors[field] = `${label} must be a number between 0 and 10,000,000.`;
      }
    });
      ...
      <Box display="flex">
        ...
        >
          ...
          >
            ...
            {/* Grid Layout for Left and Right Fields */}
